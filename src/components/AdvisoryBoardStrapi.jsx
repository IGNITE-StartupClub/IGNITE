...
const fallbackMembers = [
  {
    ...
    image: {
      url: 'https://www.leuphana.de/fileadmin/_processed_/6/5/csm_reihlen_markus__795-70697.690x690px.WEB_944517a3d2.jpg',
    },
    initials: 'MR',
  },
]

function AdvisoryMemberCard({ member, strapiUrl }) {
  const [isHovered, setIsHovered] = useState(false)

  // Get image URL - handle both Strapi media and external URLs
  const getImageUrl = () => {
    if (!member.image) return null
    if (typeof member.image === 'string') return member.image
    if (member.image.url) {
      return member.image.url.startsWith('http') ? member.image.url : `${strapiUrl}${member.image.url}`
    }
    return null
  }
          ...
          <div className="relative flex h-full w-full items-center justify-center overflow-hidden rounded-full bg-gray-900/90">
            {imageUrl ? (
              <a href={member.linkedin} target="_blank" rel="noopener noreferrer" className="h-full w-full">
                <img src={imageUrl} alt={member.name} className="h-full w-full rounded-full object-cover" />
              </a>
            ) : (
      ...
      <div className="space-y-2">
        ...
        <div className="pt-2">
          <a
            ...
            className="inline-flex items-center gap-2 rounded-full border-[var(--secondary-100)] bg-[var(--secondary-100)] px-3 py-3 text-center font-bold text-[var(--neutral-100)] shadow-[rgba(0,0,0,0.3)_0px_19px_38px,rgba(0,0,0,0.22)_0px_15px_12px] transition-all duration-150 ease-in-out hover:!text-[var(--neutral-100)] hover:-translate-y-1 hover:scale-105"
          >
            Interesse melden
            <svg
              width="16"
...
export default function AdvisoryBoardStrapi({ className = '', initialMembers = [] }) {
  const [members, setMembers] = useState(initialMembers.length > 0 ? initialMembers : fallbackMembers)
  ...
  useEffect(() => {
    // If we have initial members from SSR, don't fetch again
    ...
    async function fetchMembers() {
      try {
        const response = await fetch(`${strapiUrl}/api/advisory-board-members?populate=*&sort=order:asc`)
        if (!response.ok) throw new Error('Failed to fetch')
        const json = await response.json()

        const transformedMembers = json.data.map((item) => ({
          id: item.id,
          ...item.attributes,
      ...
      <div className="mx-auto max-w-7xl">
        ...
        {loading && (
          <div className="flex justify-center py-12">
            <div className="h-12 w-12 animate-spin rounded-full border-4 border-white/20 border-t-[var(--primary-300)]"></div>
          </div>
        )}
          ...
          <div className="mb-12 mt-12 flex max-w-full flex-wrap justify-center gap-6 overflow-x-hidden px-4 py-4 md:mb-16 md:gap-8">
            {members.map((member) => (
              <div key={member.id} className="flex w-full max-w-[280px] sm:w-auto sm:min-w-[280px] sm:max-w-[320px]">
                <AdvisoryMemberCard member={member} strapiUrl={strapiUrl} />
              </div>
            ))}
        ...
        {error && !loading && (
          <p className="mt-4 text-center text-xs text-white/40">
            Verwendung von lokalen Daten (CMS nicht verfügbar)
          </p>
        )}
      </div>
    </section>
