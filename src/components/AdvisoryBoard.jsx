  ...
  return (
    <section className={`px-4 py-16 md:py-24 ${className}`}>
      <div className="mx-auto max-w-7xl">
        ...
        {/* Board Grid */}
        <div className="mb-12 flex flex-wrap justify-center gap-6 md:mb-16 md:gap-8">
          {advisoryMembers.map((member) => (
            <div key={member.id} className="flex w-full sm:w-auto sm:min-w-[280px] sm:max-w-[320px]">
              <AdvisoryMemberCard member={member} />
            </div>
          ))}
          {/* Add Member Card */}
          <div key="add-member" className="flex w-full sm:w-auto sm:min-w-[280px] sm:max-w-[320px]">
            <AddMemberCard />
          </div>
        </div>
      </div>
