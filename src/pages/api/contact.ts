...
} from '../../templates/emails/contactFormEmail'
import {
  getContactConfirmationEmailHTML,
  getContactConfirmationSubject,
} from '../../templates/emails/contactConfirmationEmail'
...
const sendContactFormEmail = async (data: ContactFormData) => {
  ...
  await resend.emails.send({
    from: 'IGNITE Startup Club <kontakt@info.ignite-startupclub.de>',
    to: recipients,
    subject: subject,
    html: html,
  })
...
const sendConfirmationEmail = async (email: string, name?: string, isAdvisoryBoard: boolean = false) => {
  ...
  await resend.emails.send({
    from: 'kontakt@info.ignite-startupclub.de',
    to: email,
    subject: subject,
    html: html,
  })
