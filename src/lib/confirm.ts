...
async function handleApplication(db: ReturnType<MongoClient['db']>, data: any) {
  ...
  `)
  await resend.emails.send({
    from: 'Team IGNITE Startup Club <team@info.ignite-startupclub.de>',
  ...
  `)
  await resend.emails.send({
    from: 'Team IGNITE Startup Club <join@info.ignite-startupclub.de>',
    to: data.email,
...
export async function processConfirmation(token: string): Promise<{ type: string; email: string }> {
  return withDb(async (db) => {
    const { value: pending } = await db.collection<Pending>('pending_confirmations').findOneAndDelete({ token })
