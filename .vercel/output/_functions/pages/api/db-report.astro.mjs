...
async function collectReport() {
  ...
  const since14Days = new Date(Date.now() - 14 * 24 * 60 * 60 * 1e3);
  try {
...
function buildEmailHtml(report) {
  ...
  const collectionRows = report.database.collections.map(
    (c) => `
      <tr>
  ...
  const errorBlock = report.database.error ? `<div style="margin-top:16px;padding:12px 16px;background:#3f0f0f;border-left:4px solid #ef4444;border-radius:4px;font-family:monospace;font-size:13px;color:#fca5a5;word-break:break-all;">
