...
const sendFormEmail = async (data) => {
  const transporter = nodemailer.createTransport({
    ...
    auth: {
      ...
      // Dein Gmail-Benutzername
      pass: process.env.GMAIL_APP_PASSWORD
      // Dein Gmail-App-Passwort
    }
  });
  const mailOptions = {
    ...
    // Die Empfänger-Adresse für die Formulardaten
...
const sendErrorEmail = async (errorMessage) => {
  const transporter = nodemailer.createTransport({
    ...
    auth: {
      ...
      // Dein Gmail-Benutzername
      pass: process.env.GMAIL_APP_PASSWORD
      // Dein Gmail-App-Passwort (aus Schritt 2)
    }
  });
  const mailOptions = {
    from: process.env.GMAIL_USER,
    // Der Absender der E-Mail
    to: emailRecipient,
    // Die Empfänger-Adresse für Fehler
